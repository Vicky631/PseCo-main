...
class FSC147(Dataset):
    def __init__(self,
        ...
        # !HARDCODED Dec 25:
        self.data_dir = "/home/zy/wjj/PseCo-main/data/fsc147"
        ...
        self.anno_file = os.path.join(self.data_dir, f'annotation_FSC147_384_with_gt.json')
        self.data_split_file = os.path.join(self.data_dir,
                                            f'Train_Test_Val_FSC_147.json')
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train' or (self.split == 'val' and self.resize_val):
            ...
            # Iterate over each annotation and extract the 'points'
            for annotation in anno['annotations'].values():
                all_points.append(annotation['points'])
            # dots = np.array(anno['annotations'][][points])
            ...
            if self.use_additional_prompt:
                return sample['image'].float(), sample['gt_density'], sample['boxes'], sample[
                    'm_flag'], text, additional_prompt
            return sample['image'].float(), sample['gt_density'], sample['boxes'], sample['m_flag'], text
        elif self.split == "test" or self.split == "test_coco" or self.split == "val_coco" or (
                self.split == "val" and not self.resize_val):
            ...
            sample = {'image': image, 'dots': dots, 'boxes': boxes,  'gt_map': gt_map}
            return sample['image'].float(), sample['gt_map'], sample['dots']
...
def save_dataset_images(dataset, save_img_dir, save_density_dir, save_mat_dir):
    # Create directories if they don't exist
    os.makedirs(save_img_dir, exist_ok=True)
    os.makedirs(save_density_dir, exist_ok=True)
    os.makedirs(save_mat_dir, exist_ok=True)

    # Iterate through the dataset and save images, density maps, and .mat files
    for idx in tqdm(range(len(dataset))):
        # Get data for a single sample
        # sample['image'].float(), sample['gt_map'], sample['boxes'], sample['pos'], text, sample['dots']
        image, gt_density, boxes, m_flag, text, dot = dataset[idx]

        # Get the image ID (this should be unique for each image)
        im_id = dataset.idx_running_set[idx]

        # Save original image as .jpg
        img_path = os.path.join(save_img_dir, f"{im_id}.jpg")
        image_pil = Image.fromarray(image.numpy().transpose(1, 2, 0))  # Convert tensor to PIL image
        image_pil.save(img_path)

        # Save density map as .png
        density_path = os.path.join(save_density_dir, f"{im_id}.png")
        density_img = (gt_density.numpy() / (gt_density.max() + 1e-8) * 255).astype(np.uint8)  # Normalize density map
        Image.fromarray(density_img).save(density_path)

        # Save points to .mat file
        mat_path = os.path.join(save_mat_dir, f"{im_id}.mat")
        scipy.io.savemat(mat_path, dot)  # Assuming boxes represent the points for simplicity

    print("Data saving complete.")


# Example usage:
dataset = FSC147(split="train", subset_scale=1.0)


# You can use 'val' or 'test' as needed
save_dataset_images(dataset, "/home/zy/wjj/PseCo-main/data/fsc147/147/train/images", "/home/zy/wjj/PseCo-main/data/fsc147/147/train/density_maps", "/home/zy/wjj/PseCo-main/data/fsc147/147/train/points")
